...
class Student(object):
    '''
    Class for storing and retriving the Student data
    Attributes : PRN, Roll number , name , block , dob , contact , email , College name (Static Attribute)
    '''
    collegename="MITAOE"
    noofstudents=0
    try:
        def __init__(self,prn,rolln,name,block,dob,contact,email):#constructor
            self.prn = prn 
            self.__rolln = rolln 
            self.__name = name 
            self.block = block 
    ...
    def __str__(self): return str(self.prn) + str(self.rolln) + str(self.name) + str(self.block)+str(self.dob) + str(self.contact)+ str(self.email)
    def __del__(self): Student.noofstudents-=1
    @property 
    def prn(self):
        return self.__prn
    @prn.setter 
    def prn(self,prn):
        if prn != None and prn > 0 : self.__prn = prn
        else :
            print("Invalid PRN")
    @property 
    def rolln(self):
        return self.__rolln 
    @rolln.setter
    def rolln(self, rolln):
        if rolln != None and rolln > 0 : 
            self.__rolln = rolln
        else:
    ...
    def contact(self,contact):
        if contact != None and contact>1000000000 and contact < 9999999999: self.__contact = contact
        else:print("This is an invalid number")    
    @property
    def email(self):return self.__email    
    @email.setter
    def email(self,email):
        if re.match(r"\S+@\S.+[a-z]", email) != None: self.__email= email
        else: print("Invalid Email")    
    @staticmethod
    def numos():
        print("Total no. of students "+Student.noofstudents)
    @staticmethod
    def upclgname(clgn):
        if clgn.isalpha():
            Student.collegename=clgn
        else: print("Invalid Data")
    def printmembers(self):#printing function
        print ("PRN:" +str(self.prn)+ "\nRoll Number:" +str(self.rolln)+ "\nName:" + self.name +"\nBlock:" + self.block +"\nDate of Birth:"+self.dob+"\nContact Number:",self.contact,"\nE-mail:" +self.email+"\n ")
...
class Mark(Student) :
    ...
    def marklist(self,markoot,markcnt):
        if (markoot < 101 and markoot > 0) and (markcnt > 0 and markcnt < 101):
            self.__marklist={"OOT":markoot,"CN":markcnt}
        else:
...
def insertion():#function for inserting object in list as well as creating objects 
    try:
        ...
        if (marksoot < 101 and marksoot > 0) and (markscnt > 0 and markscnt < 101):
